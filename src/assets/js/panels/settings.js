...
class Settings {
    ...
    setupFolderOpeners() {
        document.querySelectorAll('.folder-opener').forEach(element => {
            const folderLink = element.getAttribute('folder');
            if(!folderLink) return;

            const fullPath = path.join(this.getMinecraftPath(), folderLink);

            element.addEventListener('click', () => {
                shell.openPath(fullPath)
                    .then(result => {
                        if(result) console.error("Erreur d'ouverture :", result);
                    });
            });
        });
    }
    ...
    setupLogExport() {
        document.getElementById("exportLogs").addEventListener("click", () => {
           ...
           const logs = Array.from(console.children)
    ...
    initAccount() {
        ...
        document.querySelector('.add-account').addEventListener('click', () => {
            document.getElementById('cancel-login').style.display = 'inline-block';
            changePanel("login");
        });
    }
    ...
    initTab() {
        ...
        document.querySelectorAll('.save-tabs-btn').forEach(button => {
        ...
        document.getElementById('settings-tab').innerHTML = `<i class="fa-solid fa-terminal"></i><span>Paramètres globaux</span>`;
    ...
    resizeSettings() {
        ...
        const absoluteSize = screenWidth - 325;

        document.querySelectorAll('.tabs-settings-content').forEach(element => {
            element.style.width = absoluteSize;
        })
    }
    ...
    async initSettingsDefault() {
        ...
        if (!(await this.database.getAll('ram')).length) {
            this.database.add({ uuid: "1234", ramMin: "3", ramMax: "4" }, 'ram')
        }
