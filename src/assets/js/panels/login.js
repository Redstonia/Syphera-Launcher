import { database, showLoginSuccess, changePanel, addAccount, accountSelect, t } from '../utils.js';
...
class Login {
    ...
    setStaticTexts() {
        ...
        if(this.database.getAll('accounts-selected').length > 0) {
    ...
    toggleLoginCards(cardType) {
        ...
        loginCard.style.display = cardType === "default" ? "flex" : "none";
        a2fCard.style.display = cardType === "a2f"? "flex" : "none";
        emailVerifyCard.style.display = cardType === "email"? "flex" : "none";
    }
    ...
    showError(elements) {
        elements.errorContent.classList.add('active');
        elements.errorContent.style.display = 'flex';
        setTimeout(() => {
            elements.errorContent.classList.remove('active');
        }, 5000);
    }
